...
public class BoardServiceImpl implements BoardService {
  ...
  public List<BoardDto> findAll() {
    ...
    return Objects.nonNull(boards) ? boardMapper.mapBoardsToBoardDTOs(boards)
  ...
  private Board getBoardByIdWithValidation(String boardId) {
    if (StringUtils.isBlank(boardId) || !boardId.matches(OBJECT_ID_REGEX)) {
      throw new InvalidIdException(format("Not valid boardID: %s", boardId));
    }

    return boardRepository.findById(boardId)
        .orElseThrow(() -> new RecordIsNotFoundException(
            format("Board is not found by id: %s", boardId)));
  }

  private Note getNoteByIdWithValidation(List<Note> notes, String noteId) {
    if (StringUtils.isBlank(noteId) || !noteId.matches(OBJECT_ID_REGEX)) {
      throw new InvalidIdException(format("Not valid noteID: %s", noteId));
    }

    if (Objects.isNull(notes) || notes.isEmpty()) {
      throw new RecordIsNotFoundException(format("No notes found by id: %s", noteId));
    }

    return notes.stream()
        .filter(note -> StringUtils.equals(note.getId(), noteId))
        .findFirst().orElseThrow(
            () -> new RecordIsNotFoundException(format("Note not found by id: %s", noteId)));
  }
